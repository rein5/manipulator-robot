...
[t, Th] = ode45(@(t, th)closedLoopTaskSpaceControl(t, th, xddot_e,yddot_e,xd_e,yd_e), time_vec, th_ic_e);
...
% integrate angles using ODE45
[t, Th] = ode45(@(t, th)closedLoopTaskSpaceControl(t, th, xddot_s,yddot_s,xd_s,yd_s), time_vec, th_ic_s);
...
function dth = closedLoopTaskSpaceControl(t, th, xddot, yddot, xd, yd)
    ...
    % end-effector actual pose (forward pos. kinematics)
    xy_ee_actual = [th(1) + l1*cos(th(3)+th(4)) + l2*cos(th(3)+th(4)+th(5)); % x + l1*cos(phi+theta1) + l2*cos(phi+theta1+theta2)
        th(2) + l1*sin(th(3)+th(4)) + l2*sin(th(3)+th(4)+th(5)); % y + l1*sin(phi+theta1) + l2*sin(phi+theta1+theta2)
        0]; 
