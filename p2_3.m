...
% minimize potential: V = th1^2 + 0.25*th2^2 + 0.66*phi^2
function dth = openLoopJointSpaceControl(t, th, xddot, yddot)
    global l1 l2 tstep K;

    % compute Jacobian 
    J = [1, 0, -l1*sin(th(3)+th(4))-l2*sin(th(3)+th(4)+th(5)), -l1*sin(th(3)+th(4))-l2*sin(th(3)+th(4)+th(5)), -l2*sin(th(3)+th(4)+th(5));
    ...
    % gradient of potential V with respect to x, y, phi, theta1, theta2
    qz = [0; 0; 1.32*th(3); 2*th(4); 0.5*th(5)];

    % compute open-loop theta dot
    t_idx = 1+int32(t/tstep);
    dth_openloop = Jpinv * [xddot(t_idx); yddot(t_idx); 0] + (eye(5) - Jpinv*J)*(-K)*qz; 
